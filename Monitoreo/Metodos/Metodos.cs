...
namespace Monitoreo
{
    class Metodos
    {
       ...
       static public void InsertLog(string IP, int Codigo)
       {
        ...
        //static public void InsertLog(string IP, int Codigo)
        //{
        //    var appSettings = ConfigurationManager.ConnectionStrings["string1"].ToString();
        //    try
        //    {
        ...
        //        Flag flag = getBanderas.GetByIP(IP, appSettings);

        //        string consulta = $"INSERT [dbo].[Logs] ([Codigo], [Fecha], [IP_Maquina], [id_Plaza]) VALUES ({Codigo.ToString()}, CAST(N' {DateTime.Now.Date.ToString("yyyy-MM-dd")} T {DateTime.Now.TimeOfDay.ToString().Substring(0, 12)}' AS DateTime), N' {IP}', {flag.id_PLaza} )";
